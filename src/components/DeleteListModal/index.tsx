...
const DeleteListModal: FC<DeleteListModalProps> = ({ listId }) => {
  ...
  const deleteListHandler = () => {
    ...
    if(list) {
      dispatch(setNotification(`Lista "${list.name}" deletetada!`, 'perigo'));
    }
  }
    ...
    <div className="modal is-active">
      ...
      <div className="modal-card">
        <header className="modal-card-head has-text-centered">
          <p className="modal-card-title">Você tem certeza que quer deletar essa lista ?</p>
        </header>
        <div className="modal-card-body">
          <h2 className="is-size-5 has-text-centered">Todas as tarefas serão deletadas</h2>
          <div className="content">
            {list?.tasks.length === 0 ?
              <p className="has-text-centered pt-4 mb-0">Não tem tarefas nessa lista!</p>
              :
              <ul>
                {list?.tasks.map(task => (
